  ...
  render: function() {
    ...
    return (
      <div className="msg-header">
        Your role right now: <span>{this.state.name}</span>
      </div>
    );
  }
});
    ...
    return (
      <div className="msg-row-container">
        <div className="msg-row">
          ...
          [Sent: <span className="msg-time">{this.props.time}</span>, by: <span className="chat-username">{this.props.username}</span>]
          </span><br/>
          {this.props.message}
        </div>
      </div>
  ...
  render: function() {
    return (
      <div className="msg-input">
        <textarea rows="5" ref="textArea" onKeyDown={this.handleUserMessage} placeholder="Type your message. Please do not use ÄÄKKÖSET. Press shift + Enter to send" />
      </div>
    );
  }
});
