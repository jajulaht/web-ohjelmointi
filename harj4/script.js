...
function naytaTalo(index) {
	...
	img.setAttribute("src",talot[index].image);
	//console.log("kuva="+talot[index].kuva);
	taloDiv.appendChild(img);
	
	var p1 = document.createElement("p");
	p1.setAttribute("class","otsikko");
	var text = document.createTextNode(talot[index].address);	
	p1.appendChild(text);

	var p2 = document.createElement("p");
	var text = document.createTextNode(talot[index].layout);	
	p2.appendChild(text);

	var p3 = document.createElement("p");
	p3.setAttribute("class","kuvaus");
	var text = document.createTextNode(talot[index].description);	
	p3.appendChild(text);

	var p4 = document.createElement("p");
	var text = document.createTextNode(talot[index].price);	
	p4.appendChild(text);
